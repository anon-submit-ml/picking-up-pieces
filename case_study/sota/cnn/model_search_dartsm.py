...
class MixedOpMinus(nn.Module):
    def __init__(self, C, stride, PRIMITIVES, beta):
        super(MixedOpMinus, self).__init__()
        self._ops = nn.ModuleList()
        self.beta = beta
        self._aux_skip = OPS['skip_connect'](C, stride, False)
    ...
    def forward(self, x, weights):
        ...
        return ret + (self.beta * self._aux_skip(x))


class CellMinus(nn.Module):

    def __init__(self, steps, multiplier, C_prev_prev, C_prev, C, reduction, reduction_prev, beta):
        super(CellMinus, self).__init__()
        ...
        for i in range(self._steps):
            for j in range(2+i):
                ...
                op = MixedOpMinus(C, stride, self.primitives[edge_index], beta)
                self._ops.append(op)
                edge_index += 1
...
class DartsMinusNetwork(nn.Module):
    def __init__(self, C, num_classes, layers, criterion, primitives, args,
                 steps=4, multiplier=4, stem_multiplier=3, drop_path_prob=0, beta=1.0):
        super(DartsMinusNetwork, self).__init__()
        #### original code
        self._C = C
        ...
        for i in range(layers):
            if i in [layers//3, 2*layers//3]:
                ...
                reduction = False
            cell = CellMinus(steps, multiplier, C_prev_prev, C_prev, C_curr, reduction, reduction_prev, beta)
