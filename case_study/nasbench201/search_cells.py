...
class NAS201SearchCell_Minus(NAS201SearchCell):
  ...
  def __init__(self, C_in, C_out, stride, max_nodes, op_names, affine=False, track_running_stats=True, beta=1):
    super(NAS201SearchCell, self).__init__()
  ...
  def MixedOp(self, node_str, x, weights):
    temp1 = sum(w * op(x) for w, op in zip(weights, self.edges[node_str]))
  ...
  def forward(self, inputs, weightss):
    ...
    for i in range(1, self.max_nodes):
      ...
      for j in range(i):
        ...
        weights  = weightss[ self.edge2index[node_str] ]
        # inter_nodes.append( sum( layer(nodes[j]) * w for layer, w in zip(self.edges[node_str], weights) ) )
        inter_nodes.append(self.MixedOp(node_str=node_str, x=nodes[j], weights=weights))
      nodes.append( sum(inter_nodes) )
    return nodes[-1]
