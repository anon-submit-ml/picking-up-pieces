...
from nasbench201.search_model_dartsm import TinyNetworkDartsMinus
from nasbench201.search_model_darts_proj import TinyNetworkDartsProj
from nasbench201.search_model_dartsm_proj import TinyNetworkDartsMinusProj
...
def main():
    ...
    if args.method in ['darts', 'blank']:
    ...
    elif args.method in ['darts-', 'blank-']:
        model = TinyNetworkDartsMinus(C=args.init_channels, N=5, max_nodes=4, num_classes=n_classes, criterion=criterion, search_space=search_space, args=args)
        ...
        model = TinyNetworkDartsMinusProj(C=args.init_channels, N=5, max_nodes=4, num_classes=n_classes, criterion=criterion, search_space=search_space, args=args, beta=0.0)
    model = model.cuda()
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if args.method in ['darts-', 'blank-'] and (args.epochs - epoch) <= 50:
            #model.set_beta = (args.epochs - epoch -1)/50.0
            beta_epoch = (args.epochs - epoch-1)/50.0
            logging.info('beta %f', beta_epoch)
            model.set_beta(beta_epoch)
        ## data aug
        if args.cutout:
