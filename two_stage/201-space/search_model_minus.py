...
class TinyNetworkMinus(nn.Module):

  def __init__(self, C, N, max_nodes, num_classes, criterion, search_space, affine=False, track_running_stats=True, k=1, beta=1.0):
    super(TinyNetworkMinus, self).__init__()
    self._C        = C
    self._layerN   = N
    self.k         = k
    self._mask     = None
    ...
    for index, (C_curr, reduction) in enumerate(zip(layer_channels, layer_reductions)):
      if reduction:
        ...
        cell = SearchCell(C_prev, C_curr, 1, max_nodes, search_space, affine, track_running_stats, k, beta)
