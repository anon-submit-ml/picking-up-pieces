...
class NAS201SearchCell_Minus(NAS201SearchCell):

  def __init__(self, C_in, C_out, stride, max_nodes, op_names, affine=False, track_running_stats=True, k=4, beta=1):
    super(NAS201SearchCell, self).__init__()
    ...
    for i in range(1, max_nodes):
      for j in range(i):
        ...
        if j == 0:
          xlists = [OPS[op_name](C_in//self.k , C_out//self.k, stride, affine, track_running_stats) for op_name in op_names]
          self.aux_skips[ node_str ] = OPS["skip_connect"](C_in, C_out, stride, affine, track_running_stats)
        else:
          xlists = [OPS[op_name](C_in//self.k , C_out//self.k,      1, affine, track_running_stats) for op_name in op_names]
          self.aux_skips[ node_str ] = OPS["skip_connect"](C_in, C_out, 1, affine, track_running_stats)
        self.edges[ node_str ] = nn.ModuleList( xlists )
    self.edge_keys  = sorted(list(self.edges.keys()))
    self.edge2index = {key:i for i, key in enumerate(self.edge_keys)}
    self.num_edges  = len(self.edges)
  
  def MixedOp(self, x, node_str, weights):
    dim_2 = x.shape[1]
    xtemp = x[ : , :  dim_2//self.k, :, :]
    xtemp2 = x[ : ,  dim_2//self.k:, :, :]
    temp1 = sum(w * op(xtemp) for w, op in zip(weights, self.edges[node_str]) if not w == 0)
    if self.k == 1:
      return temp1 + (self.aux_skips[node_str](x) * self.beta)
    ans = torch.cat([temp1,xtemp2],dim=1)
    ans = channel_shuffle(ans,self.k)
    return ans + (self.aux_skips[node_str](x) * self.beta)
  ...
  def forward(self, inputs, weightss):
    ...
    for i in range(1, self.max_nodes):
      ...
      for j in range(i):
        node_str = '{:}<-{:}'.format(i, j)
        weights  = weightss[ self.edge2index[node_str] ]
        inter_nodes.append(self.MixedOp(x=nodes[j], node_str=node_str, weights=weights))
      nodes.append( sum(inter_nodes) )
    return nodes[-1]
