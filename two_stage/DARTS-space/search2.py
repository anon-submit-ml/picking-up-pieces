...
log_file = os.path.join(args.save, 'search2_log.txt')
...
def sample(model, n):
  sampled_weights = []
  for i in range(n):
    weights_normal = utils.process_gdas_matrix(model.alphas_normal, 0.1)
    weights_reduce = utils.process_gdas_matrix(model.alphas_reduce, 0.1)
