...
from utils import process_step_vector, process_step_matrix, prune, process_gdas_matrix
...
class MixedOpMinus(nn.Module):

  def __init__(self, C, stride, k, aux_skip):
    super(MixedOpMinus, self).__init__()
    self.k = k
    self.C = C
    self._ops = nn.ModuleList()
  ...
  def forward(self, x, weights, beta):
    dim_2 = x.shape[1]
    xtemp = x[ : , :  dim_2//self.k, :, :]
    xtemp2 = x[ : ,  dim_2//self.k:, :, :]
    temp1 = sum(w * op(xtemp) for w, op in zip(weights, self._ops) if not w == 0)
    temp1 += self.aux_skip(xtemp) * beta
    if self.k == 1:
      return temp1
    #reduction cell needs pooling before concat
    if temp1.shape[2] == x.shape[2]:
...
class Cell(nn.Module):
  ...
  def forward(self, s0, s1, weights):
    ...
    offset = 0
    for i in range(self._steps):
      s = sum(self._ops[offset+j](h, weights[offset+j]) for j, h in enumerate(states) if not sum(weights[offset+j]) == 0)
      offset += len(states)
      states.append(s)
...
class CellMinus(Cell):

  def __init__(self, steps, multiplier, C_prev_prev, C_prev, C, reduction, reduction_prev, k, beta=1.0):
    super(CellMinus, self).__init__(steps, multiplier, C_prev_prev, C_prev, C, reduction, reduction_prev, k)
    self.beta = beta
    ...
    for i in range(self._steps):
      for j in range(2+i):
        stride = 2 if reduction and j < 2 else 1
        op = MixedOpMinus(C, stride, self.k, OPS["skip_connect"](C // self.k, stride, False))
        self._ops.append(op)
  ...
  def forward(self, s0, s1, weights):
    ...
    offset = 0
    for i in range(self._steps):
      s = sum(self._ops[offset+j](h, weights[offset+j], self.beta) for j, h in enumerate(states))
      offset += len(states)
      states.append(s)
...
class Network(nn.Module):
  ...
  def __init__(self, C, num_classes, layers, criterion, steps=4, multiplier=4, stem_multiplier=3, k=4,
    ...
    self.aux_skip=aux_skip
    self.tau = 10 if species in ['gumbel', 'gdas'] else None
    ...
    for i in range(layers):
      if i in [layers//3, 2*layers//3]:
        ...
        reduction = False
      if self.aux_skip:
        cell = CellMinus(steps, multiplier, C_prev_prev, C_prev, C_curr, reduction, reduction_prev, k)
      else:
  ...
  def get_softmax(self):
    weights_normal = process_step_matrix(self.alphas_normal, 'softmax', self.mask_normal)
    weights_reduce = process_step_matrix(self.alphas_reduce, 'softmax', self.mask_reduce)
  ...
  def set_mask(self, mask_n, mask_r):
    self.mask_normal = mask_n
    self.mask_reduce = mask_r
  ...
  def forward(self, input):
    ...
    if self.species == 'gdas':
      weights_normal = process_gdas_matrix(self.alphas_normal, self.tau)
      weights_reduce = process_gdas_matrix(self.alphas_reduce, self.tau)
    else:
      weights_normal = process_step_matrix(self.alphas_normal, self.species, self.mask_normal, self.tau)
      weights_reduce = process_step_matrix(self.alphas_reduce, self.species, self.mask_reduce, self.tau)
