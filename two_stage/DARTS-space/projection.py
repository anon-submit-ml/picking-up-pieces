...
def project_op(model, proj_queue, args, infer, cell_type, selected_eid=None):
    ...
    for opid in range(num_ops):
        ...
        valid_stats = infer(proj_queue, model, model._criterion, log=False, _eval=False, weights_dict=weights_dict)
...
def pt_project(train_queue, valid_queue, model, architect,
               args, infer, perturb_alpha=None, epsilon_alpha=0.3):
    model.train()
    model.printing(logging)

    train_acc, train_obj = infer(train_queue, model, model._criterion, log=False)
    logging.info('train_acc  %f', train_acc)
    logging.info('train_loss %f', train_obj)

    valid_acc, valid_obj = infer(valid_queue, model, model._criterion, log=False)
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        optimizer, float(tune_epochs), eta_min=args.learning_rate_min)
    ...
    if args.dev_resume_epoch >= 0:
        ...
        if os.path.isfile(filename):
            ...
            model.set_state_dict(optimizer, architect, scheduler, checkpoint)
    ...
    for epoch in range(start_epoch, tune_epochs):
        ...
        if epoch % proj_intv == 0 or epoch == tune_epochs - 1:
            ...
            save_state_dict = model.get_state_dict(epoch, optimizer, architect, scheduler)
        ...
        for step, (input, target) in enumerate(train_queue):
            ...
            nn.utils.clip_grad_norm_(model.get_weights(), args.grad_clip)
            optimizer.step()
    ...
    model.printing(logging)
    num_params = ig_utils.count_parameters_in_MB(model)
