...
def save_checkpoint(state, is_best, save, per_epoch=False):
  if per_epoch:
    epoch = state['epoch']
    filename = os.path.join(save, 'checkpoint_{0}.pth.tar'.format(epoch))
  else:
...
def process_gdas_matrix(x, tau):
  while True:
    gumbels = -torch.empty_like(x).exponential_().log()
    logits  = (x.log_softmax(dim=1) + gumbels) / tau
